...
set rc [catch {
  ...
  export_db {led_spi_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../constraint/pin.adc
  optimize_rtl
  report_area -file led_spi_rtl.area
  export_db {led_spi_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea led_spi_gate.area
  legalize_phy_inst
  export_db {led_spi_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
