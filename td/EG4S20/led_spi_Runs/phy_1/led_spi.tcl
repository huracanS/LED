...
set rc [catch {
  open_project {led_spi.prj}
  import_device eagle_s20.db -package EG4S20BG256
  import_db {../syn_1/led_spi_gate.db}
  config_chipwatcher "../../chipwatcher/cwc001.cwc" -dir ""
  compile_watcher
  place
  export_db {led_spi_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file led_spi_phy.area
  export_db {led_spi_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid led_spi_inst.bid
  bitgen -bit "led_spi.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
