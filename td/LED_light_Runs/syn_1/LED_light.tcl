...
set rc [catch {
  ...
  elaborate -top {fpga_ed4g}
  export_db {LED_light_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../pin.adc
  optimize_rtl
  report_area -file LED_light_rtl.area
  export_db {LED_light_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  ...
  optimize_gate -maparea LED_light_gate.area
  legalize_phy_inst
  export_db {LED_light_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
