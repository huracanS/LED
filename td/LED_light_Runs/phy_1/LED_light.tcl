...
set rc [catch {
  open_project {LED_light.prj}
  import_device eagle_s20.db -package EG4D20EG176
  import_db {../syn_1/LED_light_gate.db}
  read_sdc ../../LED_light.sdc
  place
  export_db {LED_light_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file LED_light_phy.area
  export_db {LED_light_pr.db}
  start_timer
  report_timing -mode FINAL -net_info  -rpt_autogen true -file LED_light_phy.timing
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid LED_light_inst.bid
  bitgen -bit "LED_light.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
